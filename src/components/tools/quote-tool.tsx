...
export function QuoteTool({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setData({ ...emptyQuote(), ...JSON.parse(raw) });
    } catch {
      /* ignore */
    }
    setHydrated(true);
  ...
  async function downloadPdf(unlockToken: string | null) {
    ...
    a.href = url;
    a.download = `quote-${data.number || "draft"}${unlockToken ? "" : "-preview"}.pdf`;
    ...
    <div className="grid lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <button type="button" onClick={resetDraft} className="text-sm text-[var(--muted)] hover:text-[var(--foreground)]">
            ↺ {t.reset}
          </button>
        </div>
        ...
        <div>
          <label className={labelCls}>{t.projectTitle}</label>
          <input className={inputCls} placeholder={t.projectPlaceholder} value={data.projectTitle} onChange={(e) => set("projectTitle", e.target.value)} />
        </div>

        <div className="grid sm:grid-cols-2 gap-4">
          <fieldset className="space-y-2">
            <legend className="font-semibold mb-1">{t.from}</legend>
            <input className={inputCls} placeholder={t.fromName} value={data.fromName} onChange={(e) => set("fromName", e.target.value)} />
            <textarea className={inputCls} rows={2} placeholder={t.fromDetails} value={data.fromDetails} onChange={(e) => set("fromDetails", e.target.value)} />
          </fieldset>
          <fieldset className="space-y-2">
            <legend className="font-semibold mb-1">{t.to}</legend>
            <input className={inputCls} placeholder={t.toName} value={data.toName} onChange={(e) => set("toName", e.target.value)} />
            <textarea className={inputCls} rows={2} placeholder={t.toDetails} value={data.toDetails} onChange={(e) => set("toDetails", e.target.value)} />
          </fieldset>
        </div>
        ...
        <div>
          ...
          <textarea className={inputCls} rows={3} placeholder={t.notesPlaceholder} value={data.notes} onChange={(e) => set("notes", e.target.value)} />
        </div>
      </div>
        ...
        <div className="rounded-xl border border-[var(--border)] bg-white text-slate-900 p-6 shadow-sm text-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="text-right text-xs">
              ...
              <div className="text-slate-500">{t.validUntil}</div>
              <div className="font-semibold">{data.validUntil || "—"}</div>
            </div>
          </div>
          {data.projectTitle ? <div className="font-bold mb-3">{data.projectTitle}</div> : null}
          <div className="flex justify-between gap-4 mb-4">
            ...
            <div className="w-1/2">
              ...
              <div className="text-slate-500 whitespace-pre-line text-xs">{data.toDetails}</div>
            </div>
          </div>
