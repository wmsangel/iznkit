...
import { emptyPo, lineAmount, poTotal, type PoData } from "@/lib/tools/po/model";
import { Actions } from "./hourly-tool";
import { DesignPicker } from "./design-picker";
import { getThemeAccent } from "@/lib/design/themes";

const SKU = "tool:purchase-order";
const STORAGE_KEY = "izn.tools:po:draft";
const MAX_LOGO_BYTES = 1024 * 1024;
type DownloadState = "idle" | "working" | "error" | "done";

export function PoTool({ locale }: { locale: Locale }) {
  const dict = getDictionary(locale);
  const t = dict.po;
  const tt = dict.tool;
  const [data, setData] = useState<PoData>(emptyPo);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setData({ ...emptyPo(), ...JSON.parse(raw) });
    } catch {}
    setHydrated(true);
  }, []);
  useEffect(() => {
  ...
  const totals = useMemo(() => poTotal(data), [data]);
  const accent = getThemeAccent(data.theme);
  function set<K extends keyof PoData>(key: K, value: PoData[K]) {
    setData((d) => ({ ...d, [key]: value }));
  }
  function setItem(i: number, patch: Partial<PoData["items"][number]>) {
  ...
  async function downloadPdf(unlockToken: string | null) {
    const res = await fetch("/api/pdf/po", {
      method: "POST",
    ...
    track("tool_use", { tool: "purchase-order", action: "download" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `purchase-order-${data.number || "draft"}${unlockToken ? "" : "-preview"}.pdf`;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            {data.logo ? (
              <button type="button" onClick={() => { set("logo", null); if (logoInput.current) logoInput.current.value = ""; }} className="text-sm text-[var(--muted)] hover:text-red-500">
                {t.removeLogo}
              </button>
            ) : null}
          </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <fieldset className="space-y-2">
            <legend className="font-semibold mb-1">{t.buyer}</legend>
            <input className={inputCls} placeholder={t.buyerName} value={data.buyerName} onChange={(e) => set("buyerName", e.target.value)} />
            <textarea className={inputCls} rows={2} placeholder={t.buyerDetails} value={data.buyerDetails} onChange={(e) => set("buyerDetails", e.target.value)} />
          </fieldset>
          <fieldset className="space-y-2">
            <legend className="font-semibold mb-1">{t.supplier}</legend>
            <input className={inputCls} placeholder={t.supplierName} value={data.supplierName} onChange={(e) => set("supplierName", e.target.value)} />
            <textarea className={inputCls} rows={2} placeholder={t.supplierDetails} value={data.supplierDetails} onChange={(e) => set("supplierDetails", e.target.value)} />
          </fieldset>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <input type="date" className={inputCls} value={data.deliveryDate} onChange={(e) => set("deliveryDate", e.target.value)} />
          </div>
          <div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" min={0} step="0.1" className={inputCls} value={data.taxRate} onChange={(e) => set("taxRate", Number(e.target.value))} />
          </div>
        </div>
        <div>
        ...
        <div className="rounded-xl border border-[var(--border)] bg-white text-slate-900 p-6 shadow-sm text-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="text-right text-xs">
              ...
              <div className="text-slate-500">{t.deliveryDate}</div>
              <div className="font-semibold">{data.deliveryDate || "—"}</div>
            </div>
          </div>
          <div className="flex justify-between gap-4 mb-4">
            <div className="w-1/2">
              <div className="text-[10px] uppercase text-slate-400">{t.buyer}</div>
              <div className="font-semibold">{data.buyerName || "—"}</div>
              <div className="text-slate-500 whitespace-pre-line text-xs">{data.buyerDetails}</div>
            </div>
            <div className="w-1/2">
              <div className="text-[10px] uppercase text-slate-400">{t.supplier}</div>
              <div className="font-semibold">{data.supplierName || "—"}</div>
              <div className="text-slate-500 whitespace-pre-line text-xs">{data.supplierDetails}</div>
            </div>
          </div>
