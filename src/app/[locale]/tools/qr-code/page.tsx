...
export async function generateMetadata({
  ...
  return pageMetadata({ locale, path: `tools/${SLUG}`, title: dict.qr.title, description: seo?.metaExtra ?? dict.qr.subtitle });
}
...
export default async function QrPage({
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <p className="mt-2 text-lg text-[var(--muted)] max-w-2xl">{dict.qr.subtitle}</p>
      <div className="mt-10"><QrTool locale={locale} /></div>
      {entry ? (
        <ToolContent locale={locale} slug={SLUG} toolTitle={dict.qr.title} toolBlurb={entry.tool.blurb[locale]} priceCents={entry.tool.priceCents} />
      ) : null}
    </div>
  );
}
