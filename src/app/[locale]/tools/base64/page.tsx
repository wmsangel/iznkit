...
export async function generateMetadata({
  ...
  return pageMetadata({
    locale,
    path: `tools/${SLUG}`,
    title: dict.b64.title,
    description: seo?.metaExtra ?? dict.b64.subtitle,
  });
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="mt-4 flex items-center gap-3 flex-wrap">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">{dict.b64.title}</h1>
        <span className="text-xs font-semibold rounded-full bg-[var(--accent-soft)] text-[var(--accent)] px-3 py-1">
          {dict.b64.free}
        </span>
      </div>
      <p className="mt-2 text-lg text-[var(--muted)] max-w-2xl">{dict.b64.subtitle}</p>
      <div className="mt-10">
