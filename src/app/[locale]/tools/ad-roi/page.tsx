...
import { AdRoiTool } from "@/components/tools/adroi-tool";
...
export async function generateMetadata({
  ...
  return pageMetadata({ locale, path: `tools/${SLUG}`, title: dict.adroi.title, description: seo?.metaExtra ?? dict.adroi.subtitle });
}
...
export default async function AdRoiPage({
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      {entry ? (
        <ToolContent locale={locale} slug={SLUG} toolTitle={dict.adroi.title} toolBlurb={entry.tool.blurb[locale]} priceCents={entry.tool.priceCents} />
      ) : null}
    </div>
  );
}
