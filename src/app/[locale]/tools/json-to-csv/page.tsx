...
export async function generateMetadata({
  ...
  return pageMetadata({ locale, path: `tools/${SLUG}`, title: dict.jsonCsv.title, description: seo?.metaExtra ?? dict.jsonCsv.subtitle });
}

export default async function JsonCsvPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="mt-4 flex items-center gap-3 flex-wrap">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">{dict.jsonCsv.title}</h1>
        <span className="text-xs font-semibold rounded-full bg-[var(--accent-soft)] text-[var(--accent)] px-3 py-1">{dict.jsonCsv.free}</span>
      </div>
