...
export async function generateMetadata({
  ...
  return pageMetadata({
    ...
    description: seo?.metaExtra ?? dict.tax.subtitle,
  });
}
...
export default async function TaxPage({
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <p className="mt-2 text-lg text-[var(--muted)] max-w-2xl">{dict.tax.subtitle}</p>
      <div className="mt-10">
        <TaxTool locale={locale} />
      </div>
      {entry ? (
        <ToolContent
          locale={locale}
          slug={SLUG}
          toolTitle={dict.tax.title}
