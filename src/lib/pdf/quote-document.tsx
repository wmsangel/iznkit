...
import { lineAmount, quoteTotal, type QuoteData } from "@/lib/tools/quote/model";
...
const s = StyleSheet.create({
  ...
  project: { fontSize: 13, fontWeight: "bold", marginBottom: 16 },
...
export function QuoteDocument({ data, labels, brand, watermark }: QuoteDocProps) {
  const totals = quoteTotal(data);
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.headerRow}>
          ...
          <View>
            ...
            <Text style={s.metaValue}>{data.validUntil || "—"}</Text>
          </View>
        </View>

        {data.projectTitle ? <Text style={s.project}>{data.projectTitle}</Text> : null}
