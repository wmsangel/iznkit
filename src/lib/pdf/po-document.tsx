...
import { lineAmount, poTotal, type PoData } from "@/lib/tools/po/model";
import { getThemeAccent } from "@/lib/design/themes";

export interface PoLabels {
  docTitle: string;
  number: string;
  date: string;
  deliveryDate: string;
...
const s = StyleSheet.create({
  ...
  docTitle: { fontSize: 20, fontWeight: "bold", color: ACCENT, letterSpacing: 0.5 },
  ...
  notesText: { lineHeight: 1.5, color: "#1f2937" },
...
export function PoDocument({ data, labels, brand, watermark }: PoDocProps) {
  const totals = poTotal(data);
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.headerRow}>
          ...
          <View>
            ...
            <Text style={s.metaLabel}>{labels.deliveryDate}</Text>
            <Text style={s.metaValue}>{data.deliveryDate || "—"}</Text>
          </View>
        </View>

        <View style={s.parties}>
          <View style={s.partyBlock}>
            <Text style={s.partyLabel}>{labels.buyer}</Text>
            <Text style={s.partyName}>{data.buyerName || "—"}</Text>
            <Text style={s.partyDetails}>{data.buyerDetails}</Text>
          </View>
          <View style={s.partyBlock}>
            <Text style={s.partyLabel}>{labels.supplier}</Text>
            <Text style={s.partyName}>{data.supplierName || "—"}</Text>
            <Text style={s.partyDetails}>{data.supplierDetails}</Text>
          </View>
        </View>
        ...
        <View style={s.totals}>
          ...
          {data.taxRate ? (
            <View style={s.totalRow}>
              <Text style={{ color: MUTED }}>{labels.tax} ({data.taxRate}%)</Text>
